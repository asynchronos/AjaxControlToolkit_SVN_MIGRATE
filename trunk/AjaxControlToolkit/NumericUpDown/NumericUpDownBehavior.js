...
AjaxControlToolkit.NumericUpDownBehavior.prototype = {
    ...
    readValue : function() {
        /// <summary>
        /// Parse value of textbox and rewrite this._currentValue 
        /// this._currentValue = 0 if some probleme durring parsing
        /// Parse int or string element of RefValues
        /// </summary>

        if (this._elementTextBox) {
            var v = this._elementTextBox.value;
            // if textbox empty this._currentValue = 0
            if('' == v) {
                ...
                try {
                    ...
                    this._currentValue = Number.NaN;
                }
                //if this._currentValue after parse is not a number compare to all element of _refValues 
                //else this._currentValue = 0
                if ((!this._currentValue)&&(this._currentValue != 0)) {
                    if(this._refValuesValue) {
                        var find = -1;
                    ...
                    // Is numeric but use refValue
                    // Checked index outoff range
                    if(this._refValuesValue) {
                        if ((this._currentValue<0) || (this._currentValue > this._refValuesValue.length)) {
                            this._currentValue = 0;
                        }
                    }
                }
